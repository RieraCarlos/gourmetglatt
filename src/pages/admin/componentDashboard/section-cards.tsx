import { IconTrendingDown, IconTrendingUp, IconTruck, IconPlus, IconBox, IconPercentage } from "@tabler/icons-react"
...
export function SectionCards() {
    ...
    const averageStock = useMemo(() => {
        ...
        const total = items.reduce((sum, it) => sum + it.stock, 0)
        return Math.round(total / items.length)
    }, [items])

    const totalEntries = useMemo(() => {
        return items.reduce((sum, it) => sum + it.entry, 0)
    }, [items])
